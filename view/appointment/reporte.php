    ...
    if ($exportType === 'pdf') {
        ...
        class PDF extends FPDF {
            function Header() {
                ...
                $this->Cell(80);
                $this->Cell(95,10,'Lista de las citas',1,0,'C');
                $this->Ln(20);
            }
        ...
        $result = mysqli_query($connString, "SELECT horario.codhor, horario.nomhor, doctor.nomdoc, horario.fere FROM horario INNER JOIN doctor ON horario.coddoc = doctor.coddoc") or die("database error:". mysqli_error($connString));
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $sheet->setCellValue('B' . $rowNumber, utf8_decode($row['nomhor']));
            $sheet->setCellValue('C' . $rowNumber, utf8_decode($row['nomdoc']));
